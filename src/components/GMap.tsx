...
export default function GMap({
  ...
  return (
    ...
    >
      {/* Map Tile Element with strict containment - pointer events disabled in preview mode to prevent scroll hijacking */}
      <div 
        ref={mapContainerRef} 
        className={`w-full h-full flex-1 z-0 relative ${!interactive ? "pointer-events-none select-none" : ""}`}
      />

      {/* Coordinate HUD indicator (Only shown in interactive mode or when height is large) */}
      {interactive && (
      ...
      {interactive ? (
        ...
        <div className="absolute bottom-2 right-2 z-[10] flex gap-1.5 pointer-events-auto">
          <button
            type="button"
            onClick={handleOpenMapDirections}
            className="flex items-center gap-1 bg-slate-900/90 hover:bg-slate-900 dark:bg-slate-800/90 dark:hover:bg-slate-800 active:scale-95 text-white font-bold text-[10px] px-2.5 py-1.5 rounded-lg shadow-sm backdrop-blur-xs transition-all cursor-pointer border border-white/10"
